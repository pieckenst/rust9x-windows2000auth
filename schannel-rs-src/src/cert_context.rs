...
impl Drop for CertContext {
    fn drop(&mut self) {
        eprintln!("CertContext::drop called");
        unsafe {
...
impl Clone for CertContext {
    fn clone(&self) -> CertContext {
        eprintln!("CertContext::clone called");
...
impl CertContext {
    ...
    pub fn new(data: &[u8]) -> io::Result<CertContext> {
        eprintln!("CertContext::new called with {} bytes of DER data", data.len());
        let ret = unsafe {
        ...
        if ret.is_null() {
            eprintln!("CertCreateCertificateContext failed: {:?}", io::Error::last_os_error());
            Err(io::Error::last_os_error())
        } else {
            eprintln!("CertCreateCertificateContext succeeded");
            Ok(CertContext(ret))
        }
    }
    ...
    pub fn from_pem(pem: &str) -> io::Result<CertContext> {
        eprintln!("CertContext::from_pem called with {} bytes of PEM data", pem.len());
        unsafe {
            ...
            if ok == 0 {
                eprintln!("CryptStringToBinaryA (query length) failed: {:?}", io::Error::last_os_error());
                return Err(io::Error::last_os_error());
            }
            ...
            if ok == 0 {
                eprintln!("CryptStringToBinaryA (decode) failed: {:?}", io::Error::last_os_error());
                return Err(io::Error::last_os_error());
            }

            eprintln!("PEM decoded successfully to {} bytes of DER data", len);
            CertContext::new(&buf)
        }
    }
...
impl<'a> AcquirePrivateKeyOptions<'a> {
    ...
    pub fn acquire(&self) -> io::Result<PrivateKey> {
        eprintln!("AcquirePrivateKeyOptions::acquire called with flags: 0x{:08X}", self.flags);
        unsafe {
            ...
            eprintln!("Calling CryptAcquireCertificatePrivateKey");
            ...
            if res == 0 {
                eprintln!("CryptAcquireCertificatePrivateKey failed: {:?}", io::Error::last_os_error());
                return Err(io::Error::last_os_error());
            }
            assert_ne!(free, 0);
            eprintln!("CryptAcquireCertificatePrivateKey succeeded, spec: {}", spec);
            if spec == Cryptography::CERT_NCRYPT_KEY_SPEC {
                eprintln!("Using NCRYPT key provider");
                Ok(PrivateKey::NcryptKey(NcryptKey::from_inner(handle)))
            } else {
                eprintln!("Using CryptoAPI provider");
