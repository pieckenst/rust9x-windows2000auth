...
impl Default for Builder {
    fn default() -> Builder {
        eprintln!("Builder::default called");
        Builder {
            domain: None,
            use_sni: true,
...
impl Builder {
    ...
    pub fn use_sni(&mut self, use_sni: bool) -> &mut Builder {
        eprintln!("Builder::use_sni called with use_sni: {}", use_sni);
    ...
    pub fn accept_invalid_hostnames(&mut self, accept_invalid_hostnames: bool) -> &mut Builder {
        eprintln!("Builder::accept_invalid_hostnames called with accept_invalid_hostnames: {}", accept_invalid_hostnames);
    ...
    {
        eprintln!("Builder::verify_callback called");
    ...
    pub fn cert_store(&mut self, cert_store: CertStore) -> &mut Builder {
        eprintln!("Builder::cert_store called");
    ...
    pub fn request_application_protocols(&mut self, alpns: &[&[u8]]) -> &mut Builder {
        eprintln!("Builder::request_application_protocols called with {} protocols", alpns.len());
    ...
    where
        S: Read + Write,
    {
        eprintln!("Builder::connect called - starting client TLS connection");
        log_init_requests();
    ...
    where
        S: Read + Write,
    {
        eprintln!("Builder::accept called - starting server TLS connection");
        log_accept_requests();
        ...
        S: Read + Write,
    {
        ...
        let domain = match self.domain {
            ...
            _ => {
                eprintln!("SNI disabled or no domain set");
                None
            },
        };
        let (ctxt, buf) = match SecurityContext::initialize(
            ...
            Ok(pair) => {
                eprintln!("SecurityContext::initialize succeeded");
                pair
            },
            Err(e) => {
                eprintln!("SecurityContext::initialize failed: {:?}", e);
                return Err(HandshakeError::Failure(e));
            },
        };

        eprintln!("Creating TlsStream with state: Initializing");
        ...
        eprintln!("Starting handshake process");
    ...
    S: Read + Write,
{
    ...
    pub fn shutdown(&mut self) -> io::Result<()> {
        ...
        match self.state {
            State::Shutdown => {
                eprintln!("Already in shutdown state");
                return Ok(());
            },
            State::Initializing {
                shutting_down: true,
                ...
                eprintln!("Already in initializing with shutdown");
            },
            _ => {
                eprintln!("Applying shutdown control token");
                unsafe {
                    ...
                    match Identity::ApplyControlToken(self.context.get_mut(), &desc) {
                        Foundation::SEC_E_OK => {
                            eprintln!("ApplyControlToken succeeded");
                        },
                        err => {
                            eprintln!("ApplyControlToken failed with error: 0x{:08X}", err);
        ...
        eprintln!("Calling initialize to complete shutdown");
        self.initialize().map(|_| ())
    }

    fn step_initialize(&mut self) -> io::Result<()> {
        eprintln!("TlsStream::step_initialize called");
        unsafe {
            let pos = self.enc_in.position() as usize;
            eprintln!("  Encrypted input buffer position: {}", pos);
            let mut inbufs = vec![
                secbuf(
            ...
            if let Some(ref mut alpns) = alpns {
                eprintln!("Adding ALPN buffer to input buffers");
                inbufs.push(secbuf(
            ...
            eprintln!("Output buffer count: {}", outbufs.len());
            let mut outbuf_desc = secbuf_desc(&mut outbufs);

            let mut attributes = 0;

            let status = if self.server {
                eprintln!("Calling AcceptSecurityContext (server mode)");
                let ptr = if self.accept_first {
                    eprintln!("  First call - context is NULL");
                    ptr::null_mut()
                } else {
                    eprintln!("  Subsequent call - using existing context");
                    self.context.get_mut()
                };
                eprintln!("  ACCEPT_REQUESTS: 0x{:08X}", ACCEPT_REQUESTS);
            ...
            eprintln!("Security context call returned status: 0x{:08X}", status);
            eprintln!("  Attributes: 0x{:08X}", attributes);

            for buf in &outbufs[1..] {
                if !buf.pvBuffer.is_null() {
                    eprintln!("Freeing context buffer at {:p}", buf.pvBuffer);
            ...
            match status {
                Foundation::SEC_E_OK => {
                    eprintln!("Handshake completed successfully (SEC_E_OK)");
                    let nread = if inbufs[1].BufferType == Identity::SECBUFFER_EXTRA {
                        eprintln!("  SECBUFFER_EXTRA detected, consuming {} bytes", inbufs[1].cbBuffer);
                        self.enc_in.position() as usize - inbufs[1].cbBuffer as usize
                    } else {
                        eprintln!("  No SECBUFFER_EXTRA, consuming all input");
                        self.enc_in.position() as usize
                    };
                    let to_write = if outbufs[0].pvBuffer.is_null() {
                        eprintln!("  No output data to write");
                        None
                    } else {
                        eprintln!("  Output data available: {} bytes", outbufs[0].cbBuffer);
                ...
                Foundation::SEC_I_CONTINUE_NEEDED => {
                    eprintln!("Handshake continuation needed (SEC_I_CONTINUE_NEEDED)");
                    ...
                    eprintln!("  Output data to write: {} bytes", to_write.0.cbBuffer);
                ...
                Foundation::SEC_E_INCOMPLETE_MESSAGE => {
                    eprintln!("Incomplete message (SEC_E_INCOMPLETE_MESSAGE)");
                    self.needs_read = if inbufs[1].BufferType == Identity::SECBUFFER_MISSING {
                        eprintln!("  SECBUFFER_MISSING - need {} more bytes", inbufs[1].cbBuffer);
                        inbufs[1].cbBuffer as usize
                    } else {
                        eprintln!("  No SECBUFFER_MISSING - need 1 more byte");
                        1
                    };
                }
                err => {
                    eprintln!("Security context call failed with error: 0x{:08X}", err);
                    return Err(io::Error::from_raw_os_error(err));
                },
            }
            Ok(())
    ...
    fn initialize(&mut self) -> io::Result<Option<Identity::SecPkgContext_StreamSizes>> {
        eprintln!("TlsStream::initialize called");
        loop {
            match self.state {
                State::Initializing {
                    mut needs_flush,
                    ...
                    eprintln!("  State: Initializing");
                    eprintln!("    needs_flush: {}", needs_flush);
                    eprintln!("    more_calls: {}", more_calls);
                    eprintln!("    shutting_down: {}", shutting_down);
                    eprintln!("    validated: {}", validated);

                    if self.write_out()? > 0 {
                        eprintln!("  Wrote data, setting needs_flush = true");
                        needs_flush = true;
                        if let State::Initializing {
                            ref mut needs_flush,
                            ..
                    ...
                    if !shutting_down && !validated {
                        eprintln!("  Validating certificate (require_cert: {})", !more_calls);
                        ...
                        if self.validate(!more_calls)? {
                            ...
                            {
                                *validated = true;
                                eprintln!("  Certificate validated successfully");
                            }
                        }
                    }

                    if !more_calls {
                        eprintln!("  No more calls needed, transitioning to final state");
                        self.state = if shutting_down {
                            eprintln!("  Transitioning to Shutdown state");
                            State::Shutdown
                        } else {
                            eprintln!("  Transitioning to Streaming state");
                            let sizes = self.context.stream_sizes()?;
                            eprintln!("    Stream sizes: header={}, trailer={}, max_msg={}", 
                    ...
                    if self.needs_read > 0 {
                        eprintln!("  Need to read {} bytes", self.needs_read);
                        if self.read_in()? == 0 {
                            eprintln!("  Unexpected EOF during handshake");
                    ...
                    eprintln!("  Calling step_initialize");
                    self.step_initialize()?;
                }
                State::Streaming { sizes } => {
                    eprintln!("  Already in Streaming state");
    ...
    fn validate(&mut self, require_cert: bool) -> io::Result<bool> {
        eprintln!("TlsStream::validate called with require_cert: {}", require_cert);
        // If we're accepting connections then we don't perform any validation
        // for the remote certificate, that's what they're doing!
        if self.server {
            eprintln!("  Server mode - skipping certificate validation");
            return Ok(false);
        }

        eprintln!("  Client mode - validating remote certificate");
        let cert_context = match self.context.remote_cert() {
            Err(_) if !require_cert => {
                eprintln!("  Certificate not required yet, skipping validation");
                return Ok(false);
            },
            ret => {
                eprintln!("  Retrieved remote certificate context");
                ret?
            },
        };
        ...
        let cert_chain = unsafe {
            let cert_store = match (cert_context.cert_store(), &self.cert_store) {
                (Some(ref mut chain_certs), &Some(ref extra_certs)) => {
                    eprintln!("  Adding {} extra certificates to chain store", extra_certs.certs().count());
                ...
                (Some(chain_certs), &None) => {
                    eprintln!("  Using chain cert store only");
                    chain_certs.as_inner()
                },
                (None, &Some(ref extra_certs)) => {
                    eprintln!("  Using extra cert store only");
                    extra_certs.as_inner()
                },
                (None, &None) => {
                    eprintln!("  No cert store available");
                    ptr::null_mut()
                },
            };
            ...
            eprintln!("  CertGetCertificateChain flags: 0x{:08X}", flags);
            ...
            eprintln!("  Certificate usage identifiers: {}", identifiers.len());

            let mut cert_chain = mem::zeroed();

            eprintln!("  Calling CertGetCertificateChain");
            ...
            if res != 0 {
                eprintln!("  CertGetCertificateChain succeeded");
                CertChainContext(cert_chain)
            } else {
                eprintln!("  CertGetCertificateChain failed: {:?}", io::Error::last_os_error());
        ...
        unsafe {
            ...
            if let Some(ref mut store) = self.cert_store {
                if let Some(chain) = cert_chain.final_chain() {
                    ...
                    {
                        eprintln!("  Found trusted certificate in custom store");
            ...
            eprintln!("  Calling CertVerifyCertificateChainPolicy with SSL policy");
            ...
            if res == 0 {
                eprintln!("  CertVerifyCertificateChainPolicy failed: {:?}", io::Error::last_os_error());
                return Err(io::Error::last_os_error());
            }

            eprintln!("  Certificate verification status: 0x{:08X}", status.dwError);
            let mut verify_result = if status.dwError != Foundation::ERROR_SUCCESS {
                eprintln!("  Certificate verification failed with error: 0x{:08X}", status.dwError);
                Err(io::Error::from_raw_os_error(status.dwError as i32))
            } else {
                eprintln!("  Certificate verification succeeded");
                Ok(())
            };

            // check if there's a user-specified verify callback
            if let Some(ref callback) = self.verify_callback {
                eprintln!("  Calling user verification callback");
        ...
        eprintln!("  Certificate validation completed successfully");
        Ok(true)
    }

    fn write_out(&mut self) -> io::Result<usize> {
        if self.is_renegotiating {
            eprintln!("  write_out: renegotiating, skipping write");
            return Ok(0);
        }
        let out_buf_len = self.out_buf.get_ref().len();
        let out_buf_pos = self.out_buf.position() as usize;
        eprintln!("  write_out: {} bytes to write (position: {}, total: {})", 
        ...
        while self.out_buf.position() as usize != self.out_buf.get_ref().len() {
            ...
            eprintln!("  write_out: wrote {} bytes this call, total: {}", nwritten, out);
        }

        eprintln!("  write_out: completed, wrote {} bytes total", out);
        Ok(out)
    }

    fn read_in(&mut self) -> io::Result<usize> {
        eprintln!("  read_in called, needs_read: {}", self.needs_read);
        let mut sum_nread = 0;
        ...
        while self.needs_read > 0 {
            ...
            if self.enc_in.get_ref().len() < min_len {
                eprintln!("    Resizing buffer from {} to {}", self.enc_in.get_ref().len(), min_len);
            ...
            eprintln!("    Read {} bytes, remaining needs_read: {}", nread, self.needs_read);
            if nread == 0 {
                eprintln!("    Read returned 0 bytes (EOF)");
                break;
            }
            sum_nread += nread;
        }

        eprintln!("  read_in completed, total bytes read: {}", sum_nread);
        Ok(sum_nread)
    }
    ...
    fn decrypt(&mut self) -> io::Result<bool> {
        eprintln!("  decrypt called");
        unsafe {
            let position = self.enc_in.position() as usize;
            eprintln!("    Encrypted data length: {}", position);
            let mut bufs = [
                secbuf(
            ...
            match Identity::DecryptMessage(self.context.get_mut(), &bufdesc, 0, ptr::null_mut()) {
                Foundation::SEC_E_OK => {
                    ...
                    let nread = if bufs[3].BufferType == Identity::SECBUFFER_EXTRA {
                        eprintln!("    SECBUFFER_EXTRA detected: {} bytes", bufs[3].cbBuffer);
                        self.enc_in.position() as usize - bufs[3].cbBuffer as usize
                    } else {
                        eprintln!("    No SECBUFFER_EXTRA");
                        self.enc_in.position() as usize
                    };
                    self.consume_enc_in(nread);
                    self.needs_read = (self.enc_in.position() == 0) as usize;
                ...
                Foundation::SEC_E_INCOMPLETE_MESSAGE => {
                    eprintln!("    DecryptMessage returned SEC_E_INCOMPLETE_MESSAGE");
                    self.needs_read = if bufs[1].BufferType == Identity::SECBUFFER_MISSING {
                        eprintln!("    SECBUFFER_MISSING: need {} more bytes", bufs[1].cbBuffer);
                ...
                Foundation::SEC_I_CONTEXT_EXPIRED => {
                    eprintln!("    DecryptMessage returned SEC_I_CONTEXT_EXPIRED");
                    Ok(true)
                }
                Foundation::SEC_I_RENEGOTIATE => {
                    eprintln!("    DecryptMessage returned SEC_I_RENEGOTIATE");
                    self.state = State::Initializing {
                        needs_flush: false,
                        more_calls: true,
                        shutting_down: false,
                    ...
                    if (self.out_buf.position() as usize) < self.out_buf.get_ref().len() {
                        eprintln!("    Setting renegotiation flag (unsent data in out_buf)");
                        self.is_renegotiating = true;
                    }

                    let nread = if bufs[3].BufferType == Identity::SECBUFFER_EXTRA {
                        eprintln!("    SECBUFFER_EXTRA during renegotiate: {} bytes", bufs[3].cbBuffer);
                        self.enc_in.position() as usize - bufs[3].cbBuffer as usize
                    } else {
                        self.enc_in.position() as usize
                    };
                ...
                err => {
                    eprintln!("    DecryptMessage failed with error: 0x{:08X}", err);
    ...
    fn encrypt(
        ...
        eprintln!("  encrypt called with {} bytes of data", buf.len());
        ...
        unsafe {
            ...
            eprintln!("    Total encrypted size: header={} + data={} + trailer={} = {}", 
                   sizes.cbHeader, buf.len(), sizes.cbTrailer, len);

            if self.out_buf.get_ref().len() < len {
                eprintln!("    Resizing output buffer to {} bytes", len);
    ...
    S: Read + Write,
{
    ...
    pub fn handshake(mut self) -> Result<TlsStream<S>, HandshakeError<S>> {
        eprintln!("MidHandshakeTlsStream::handshake called");
        match self.inner.initialize() {
            Ok(_) => {
                eprintln!("Handshake completed successfully");
                Ok(self.inner)
            },
            Err(ref e) if e.kind() == io::ErrorKind::WouldBlock => {
                eprintln!("Handshake interrupted (WouldBlock)");
